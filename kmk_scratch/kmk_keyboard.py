...
class KMKKeyboard:
	...
	_resume_buffer = []
	_resume_buffer_x = []
	_this_key_press_ms = 0
	_last_key_press_ms = 0
	_this_key_release_ms = 0
	_last_key_release_ms = 0

	# this should almost always be PREpended to, replacesf
	...
	def _send_hid(self) -> None:
		...
		if debug.enabled:
			if self.keys_pressed:
				...
				#print(f'now = {ticks_ms()}, last tick was: {self.last_tick}')
			if self.axes:
	...
	def _handle_matrix_report(self, kevent: KeyEvent) -> None:
		...
		if kevent is not None:
			#print(f'press_1: {ticks_ms()}')
			self._last_key_press_ms = self._this_key_press_ms
			self._this_key_press_ms = ticks_ms()
			#print(f'last_press_ts: {self._last_key_press_ms}')
			#print(f'this_press_ts: {self._this_key_press_ms}')
	...
	def process_key(
		...
		if is_pressed:
			...
			self._last_key_release_ms = ticks_ms()
			#print(f'release_ts: {self._last_key_release_ms}')
	...
	def set(self, ms: int) -> None:
		self._this_key_press_ms = ms

	#def set_last_key_press_ms(self, ms: int) -> None:
	#    self._last_key_press_ms = ms

	def get_last_key_press_ms(self) -> int:
		return self._last_key_press_ms

	def get_this_key_press_ms(self) -> int:
		return self._this_key_press_ms
